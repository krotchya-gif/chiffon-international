  ...
  return (
    <section className="container-edge py-24 md:py-32">
      <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
        <SectionHeading
          eyebrow={t.subPerusahaanSection.eyebrow}
          title={t.subPerusahaanSection.title}
          description={t.subPerusahaanSection.description}
        />
        <Reveal delay={0.1}>
          <Link
            href={localizeHref("/sub-perusahaan", locale)}
            className="group inline-flex items-center gap-2 text-sm font-medium text-accent hover:underline underline-offset-4"
          >
            {t.subPerusahaanSection.lihatSemua}
            <ArrowRight
              weight="light"
        ...
        {isError && (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-muted/40 px-6 py-20 text-center">
            ...
            <p className="mt-4 font-display text-xl font-semibold">
              {t.subPerusahaanSection.errorTitle}
            </p>
            <p className="mt-2 max-w-sm text-sm text-muted-foreground">
              {t.subPerusahaanSection.errorDesc}
            </p>
            <button
              type="button"
              onClick={() => refetch()}
            ...
            >
              {t.subPerusahaanSection.cobaLagi}
            </button>
          </div>
        )}
